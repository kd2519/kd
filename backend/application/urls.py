...
from brain_start.views import (
    runoob, eeg, serve_report, import_eeg_data, analyze_existing_data,
    test_api_key, latest_eeg_record_json, all_eeg_records_json
)
# =========== 初始化系统配置 =================
...
urlpatterns = (
        [
            ...
            path("api/password/send_reset_code/", SendResetCodeView.as_view()),
            path("api/password/reset_by_code/", ResetPasswordByCodeView.as_view()),
            ...
            path('brain/', runoob),  # 注意：runoob 可能是一个首页，根据实际情况调整
            path('brain/eeg/', eeg),
            path('brain/api/import-eeg-data/', import_eeg_data),
            path('brain/api/analyze-existing-data/', analyze_existing_data),
            path('brain/api/test-api-key/', test_api_key),
            path('brain/api/latest-eeg-record/', latest_eeg_record_json),
            path('brain/api/all-eeg-records/', all_eeg_records_json),
            re_path(r'^brain/reports/(?P<filename>[^/]+)$', serve_report),
        ]
